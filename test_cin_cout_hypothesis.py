...
Generates multiple BSM with different parameters c_in, c_out and tests the quality in terms of the ratio
"""
...
def misclustering_MonteCarloBasic(min = 0.2, max = 0.6, nb_X = 100, nb_Simu = 100, random = False):
    ...
    for i in range(nb_X):
        ...
        for j in range(nb_Simu):
           A, classes = bsm.simulate(n, cin, X[i]*cin, k, classes = classes)
