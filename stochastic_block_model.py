...
def simulate_Importance_Sampling(n, cin, cout,qin,qout, k, set_of_vertices, Classes, model):
    ...
    for i in set_of_vertices:
        for j in range(n):
            modified_matrix[i,j] = qin*int((matrix[i,j]==cin/n)) + qout*int((matrix[i,j]== cout/n)) #Change of probability
